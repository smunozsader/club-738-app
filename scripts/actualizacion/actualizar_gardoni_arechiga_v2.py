...
NOTA: Folios de armas de ARECHIGA pendientes (se agregarán después)
"""
...
if not arechiga_rows.empty:
    ...
    if c647155_existe.empty:
        ...
        df = pd.concat([df, pd.DataFrame([nueva_arma_1])], ignore_index=True)
        print("✅ CZ P07 C647155 agregada (FOLIO pendiente)")
    else:
        print("⚠️  CZ P07 C647155 ya existe")
    
    # Arma 2: Grand Power LP380 K078999
    k078999_existe = df[df['MATRÍCULA'] == 'K078999']
    if k078999_existe.empty:
        print("\n➕ Agregando: Grand Power LP380 K078999")
        nueva_arma_2 = {
            'No. REGISTRO DEL CLUB': arechiga_base.get('No. REGISTRO DEL CLUB', ''),
            'DOMICILIO DEL CLUB': arechiga_base.get('DOMICILIO DEL CLUB', ''),
            'No. CREDENCIAL': arechiga_base['No. CREDENCIAL'],
        ...
        print("✅ Grand Power LP380 K078999 agregada (FOLIO pendiente)")
    else:
...
if not k084328_rows.empty:
    ...
    print(f"📍 K084328 encontrada")
    print(f"   Propietario actual: {propietario_actual}")
    
    if propietario_actual == 'jrgardoni@gmail.com':
        print("🔄 REASIGNANDO de Gardoni a Arechiga...")
        
        # Obtener datos de Arechiga nuevamente por si se agregaron armas arriba
        arechiga_rows = df[df['EMAIL'] == 'arechiga@jogarplastics.com']
        if not arechiga_rows.empty:
            arechiga_base = arechiga_rows.iloc[0]
            ...
            df.loc[df['MATRÍCULA'] == 'K084328', 'No. CONSEC. DE SOCIO'] = arechiga_base.get('No. CONSEC. DE SOCIO', '')
...
for idx, arma in arechiga_final.iterrows():
    folio_status = '⚠️ PENDIENTE' if not arma.get('FOLIO') else arma['FOLIO']
    print(f"   • {arma['CLASE']:20} {arma.get('CALIBRE', 'N/A'):12} {arma['MARCA']:20} MAT: {arma['MATRÍCULA']:15} FOLIO: {folio_status}")
