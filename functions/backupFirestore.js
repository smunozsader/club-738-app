...
exports.manualFirestoreBackup = onCall(
    ...
    async (request) => {
      ...
      const outputUriPrefix =
        `gs://${BUCKET_NAME}/firestore-backups/manual-${timestamp}`;
      ...
      try {
        ...
        console.log(
            `✅ Backup manual iniciado por ${request.auth.token.email}`,
        );
...
exports.restoreFirestoreBackup = onCall(
    ...
    async (request) => {
      ...
      try {
        ...
        console.log(`⚠️ Iniciado por: ${request.auth.token.email}`);
...
exports.listFirestoreBackups = onCall(
    ...
    async (request) => {
      ...
      try {
        ...
        const backupList = Object.values(backups).sort(
        ...
        throw new HttpsError("internal", "List backups failed");
      }
    },
);
